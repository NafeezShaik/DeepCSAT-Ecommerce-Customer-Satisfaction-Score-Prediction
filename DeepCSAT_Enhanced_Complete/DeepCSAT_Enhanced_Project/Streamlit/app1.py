...
# -----------------------
# Get base directory (works on any PC)
# -----------------------
...
@st.cache_resource
def load_model():
    model_path = os.path.join(BASE_DIR, "csat_model.h5")
...
with tab1:
    ...
    channel_name           = st.sidebar.text_input("Channel Name", "Inbound")
    ...
    if st.button("🚀 Predict Single CSAT Score"):
        ...
        try:
            ...
            numerical_features = [
                'Item_price', 'connected_handling_time', 'Response_Time_seconds',
            ...
            new_data_processed[numerical_features] = scaler.transform(
                new_data_processed[numerical_features])
            ...
            csat_score = int(pred_classes[0]) + 1

            # Result display
            st.success(f"✅ Predicted CSAT Score: **{csat_score} / 5**")

            # Emoji indicator
            emoji_map = {1: "😡 Very Unsatisfied", 2: "😞 Unsatisfied",
                         3: "😐 Neutral", 4: "😊 Satisfied", 5: "😍 Very Satisfied"}
            st.markdown(f"### {emoji_map.get(csat_score, '')}")
...
with tab2:
    ...
    st.markdown("Upload a CSV file with customer interaction records to get predictions for all rows.")

    # Show sample format
    with st.expander("📋 View expected CSV column format"):
        st.code("""channel_name, category, Sub-category, order_date_time,
Issue_reported at, issue_responded, Customer_City,
Product_category, Item_price, connected_handling_time,
Agent_name, Supervisor, Manager, Tenure Bucket,
Agent Shift, Survey_response_Date""")
        ...
        if st.button("🔍 Run Batch Prediction"):
            try:
                with st.spinner("Running predictions on all records..."):
                    ...
                    df_processed[numerical_features] = scaler.transform(
                ...
                col2.metric("Avg CSAT Score",   round(df["Predicted_CSAT"].mean(), 2))
                col3.metric("Most Common CSAT", int(df["Predicted_CSAT"].mode()[0]))

                # Distribution chart
                st.markdown("#### CSAT Score Distribution")
                score_counts = df["Predicted_CSAT"].value_counts().sort_index()
                st.bar_chart(score_counts)

                # Preview results
                st.markdown("#### Preview Results")
                st.dataframe(df[["Predicted_CSAT"]].join(
                    df.drop(columns=["Predicted_CSAT"])).head(10))
                ...
                st.info("💡 Make sure your CSV has the correct column names as shown above.")
